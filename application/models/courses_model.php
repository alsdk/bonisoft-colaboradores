...
class courses_model extends CI_Model {
    ...
    function get_course($course_id) {
        $query_cursos = "SELECT course.* 
                         FROM collaborators_courses course
	...
	/**
	 * Insert a new COLLABORATOR course and student
         * 
	...
	function insert_course() {
			...
			// INSERT INTO `jomhvvkj_wrdp1`.`collaborators_courses` (`course_date` , `course_upro_asoc_id` , `course_students_id`) VALUES ('2014-03-04', '22', 'AFU312')	
            $sql = "INSERT INTO `jomhvvkj_wrdp1`.`collaborators_courses` 
...
//            die();
        ...
        function get_courses_colab($coupon_id = null) {
            ...
            /* ORIGINAL QUE FUNCIONA
                      $query_cursos = "SELECT c.course_upro_asoc_id,
                                       ...
                                       FROM collaborators_courses AS c
        ...
        function check_exist_course_collab($course_date, $course_students_id, $course_upro_asoc_id) {
            ...
            $sql = "SELECT * 
                    FROM  `jomhvvkj_wrdp1`.`collaborators_courses`
      ...
      /**
      * Obtiene un curso de la tabla courses dado un course_dates_id
      * 
      * @author Antonio Rosado
      * @return array de arrays con resultados de la query
      */
     //function get_collaborators_course($course_id = NULL) {
     function get_courses() {
        // Do the query         
        $query_cursos = "SELECT * FROM courses";
        // Return the query
        return $this->db->query($query_cursos);
   }
     
     
        /**
        * Inserta un curso de la tabla Courses
        * 
        * @author Antonio Rosado
        * 
        */        
        function insert_course_dates() {
            ...
            $course_dates_fin = ""; //no usado por ahora pero lo introducimos vacío
            $course_dates_descripcion = $this->input->post('course_description');
            //print_r($course_dates_descripcion);
            
            $sql = "INSERT INTO `jomhvvkj_wrdp1`.`courses` 
...
//            
//            return $existe;
//        print_r($existe = $response->num_rows() > 0);
//        die();
            return $existe = $response->num_rows() > 0;
        ...
        /**
         * 
         * @param type $course_dates_id id del curso a eliminar
         */
        function delete_course($course_dates_id ){
            $sql = "DELETE FROM  `jomhvvkj_wrdp1`.`courses` 
                    WHERE course_dates_id = " . $course_dates_id;
