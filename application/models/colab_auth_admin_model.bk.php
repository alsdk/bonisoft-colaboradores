				...
				if ($current_status[$user_id] != $status)
				{
					...
					else //     DESUSPENDE LA CUENTA
					{
                                             ...
                                             $this->flexi_auth->update_user($user_id, array($this->flexi_auth->db_column('user_acc', 'suspend') => 0));
                                                ...
                                                $sql = "SELECT upro_id, uacc_id, upro_first_name, upro_last_name, upro_descuento, upro_comision, uacc_email
						...
						AND upro_id = " . $user_id;
                                                    ...
                                                    if ($query->num_rows()>0)
                                                    {
                                                        foreach ($query->result() as $row)
                                                        {
                                                            ...
                                                            $template = $this->CI->auth->email_settings['email_template_directory'].$this->CI->auth->email_settings['email_template_welcome_email'];
                                                            
                                                            $this->CI->flexi_auth_model->send_email($email_to, $email_title, $user_data, $template);
                                                            
                                                            //print_r($row->upro_first_name);
                                                            //print_r($row->upro_last_name);
                                                            //print_r($row->upro_descuento);
                                                            //print_r($row->upro_comision);
