...
class Auth_admin extends CI_Controller {
	...
	/**
	 * Many of the functions within this controller load a custom model called 'colab_auth_admin_model' that has been created for the purposes of this application "COLABORADORES".
	 * The 'colab_auth_admin_model' file is not part of the flexi auth library, it is included to demonstrate how some of the functions of flexi auth can be used.
	 *
	...
	*/
    ...
    function dashboard()
    {
		...
		//$this->load->view('colab/admin/dashboard_view', $this->data);
    ...
    function manage_user_accounts()
    {
		...
		{
			$this->colab_auth_admin_model->update_user_accounts();
		}

		// Get user account data for all users. 
		// If a search has been performed, then filter the returned users.
		//$this->colab_auth_admin_model->get_user_accounts();
                $this->colab_auth_admin_model->get_colaboradores_listado();
		...
		$this->data['message'] = (! isset($this->data['message'])) ? $this->session->flashdata('message') : $this->data['message'];		
                 ...
                 * Add tittle.
                 */
                $this->data['titulo'] = "Manage User Accounts";
		$this->load->view('colab/admin/user_acccounts_view', $this->data);		
    }
	...
	function update_user_account($user_id)
	{
		...
		{
			$this->load->model('colab_auth_admin_model');
			$this->colab_auth_admin_model->update_user_account($user_id);
			
			//Get user suspension status
                        $sql = "SELECT uacc_suspend FROM user_accounts WHERE uacc_id = " . $user_id;
                        ...
                        if ($query->num_rows()>0)
                        {
                            foreach ($query->result() as $row)
                            {
                                ...
                                $suspension_status = $row->uacc_suspend;                                                    

                            }
                        }
                        //If discount and comision have been filed then dissuspenaccount
                        if ($suspension_status==1 && ($this->input->post('update_descuento')!=0 && $this->input->post('update_comision')!=0))
                        {
                            //Set user account status = unsuspended
                            $this->flexi_auth->update_user($user_id, array($this->flexi_auth->db_column('user_acc', 'suspend') => 0));
                            //Send email to the user informing that he is able to login
		...
		// Set any returned status/error messages.
		$this->data['message'] = (! isset($this->data['message'])) ? $this->session->flashdata('message') : $this->data['message'];		
                $this->data['titulo'] = 'Actualizar cuenta';
                
		$this->load->view('colab/admin/user_account_update_view', $this->data);
	}
    ...
    function manage_user_groups()
    {
		...
		{
			$this->load->model('colab_auth_admin_model');
			$this->colab_auth_admin_model->manage_user_groups();
		}
		...
		$this->data['message'] = (!isset($this->data['message'])) ? $this->session->flashdata('message') : $this->data['message'];		
                 ...
                 * Add tittle.
                 */
                $this->data['titulo'] = "Manage User Groups";
		$this->load->view('colab/admin/user_groups_view', $this->data);		
    }
	...
	function insert_user_group()
	{
		...
		{
			$this->load->model('colab_auth_admin_model');
			$this->colab_auth_admin_model->insert_user_group();
		}
		
		// Set any returned status/error messages.
		$this->data['message'] = (!isset($this->data['message'])) ? $this->session->flashdata('message') : $this->data['message'];		

		$this->load->view('colab/admin/user_group_insert_view', $this->data);
	}
	...
	function update_user_group($group_id)
	{
		...
		{
			$this->load->model('colab_auth_admin_model');
			$this->colab_auth_admin_model->update_user_group($group_id);
		}
		...
		$this->load->view('colab/admin/user_group_update_view', $this->data);
	}
    ...
    function manage_privileges()
    {
		...
		{
			$this->load->model('colab_auth_admin_model');
			$this->colab_auth_admin_model->manage_privileges();
		}
		...
		$this->data['message'] = (!isset($this->data['message'])) ? $this->session->flashdata('message') : $this->data['message'];		
                 ...
                 * Add tittle.
                 */
                $this->data['titulo'] = "Manage Privileges";
             
		$this->load->view('colab/admin/privileges_view', $this->data);
	}
	...
	function insert_privilege()
	{
		...
		{
			$this->load->model('colab_auth_admin_model');
			$this->colab_auth_admin_model->insert_privilege();
		}
		
		// Set any returned status/error messages.
		$this->data['message'] = (!isset($this->data['message'])) ? $this->session->flashdata('message') : $this->data['message'];		

		$this->load->view('colab/admin/privilege_insert_view', $this->data);
	}
	...
	function update_privilege($privilege_id)
	{
		...
		{
			$this->load->model('colab_auth_admin_model');
			$this->colab_auth_admin_model->update_privilege($privilege_id);
		}
		...
		$this->load->view('colab/admin/privilege_update_view', $this->data);
	}
    ...
    function update_user_privileges($user_id)
    {
                 ...
                 * Fixed title
                 */
               $this->data['titulo'] = "Update User Privileges";
		...
		{
			$this->load->model('colab_auth_admin_model');
			$this->colab_auth_admin_model->update_user_privileges($user_id);
		}
		...
		$sql_select = array(
			...
			'upro_first_name', 
			'upro_last_name',
                        // @author Mariano Salinas Add necessary column
                        'upro_asoc_id',
		...
		$this->load->view('colab/admin/user_privileges_update_view', $this->data);		
    }
    ...
    function update_group_privileges($group_id)
    {
		...
		{
			$this->load->model('colab_auth_admin_model');
			$this->colab_auth_admin_model->update_group_privileges($group_id);
		}
		...
		$this->load->view('colab/admin/user_group_privileges_update_view', $this->data);		
    }
	...
	function list_user_status($status = FALSE)
	{
		...
		// The view associated with this controller method is used by multiple methods, therefore set a page title.
                 ...
                 * Correct tittle.
                 */
		$this->data['titulo'] = ($status == 'inactive') ? 'Inactive Users' : 'Active Users';
		...
		$sql_select = array(
			...
			$this->flexi_auth->db_column('user_group', 'name'),
			// The following columns are located in the demo example 'collaborators_user_profiles' table, which is not required by the library.
			'upro_first_name', 
			'upro_last_name'
		);
		...
		$this->load->view('colab/admin/users_view', $this->data);
	}
	...
	function delete_unactivated_users()
	{
		...
		$this->data['users'] = $this->flexi_auth->get_unactivated_users_array($inactive_days, $sql_select);
                 ...
                 * Add tittle.
                 */
                $this->data['titulo'] = "List Unactivated Users";
		$this->load->view('colab/admin/users_unactivated_view', $this->data);
	}
	...
	function failed_login_users()
	{
		...
		// The view associated with this controller method is used by other methods, therefore set a page title.
                 ...
                 * Correct tittle.
                 */
		$this->data['titulo'] = 'Failed Login Users';
