<?php
/**
 * COLLABORATORS managament web application made in PHP with CodeInginter 
...
class Auth extends CI_Controller {
	...
	###++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++###	
	// COLABORADORES powered by flexi auth
	###++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++###	

	/**
	 * Many of the functions within this controller load a custom model
         *  called 'colab_auth_model' that has been created for the purposes 
         * of this application "COLABORADORES".
	 * The 'colab_auth_model' file is not part of the flexi auth library.
	 */
	 ...
	 *  accounts that can be logged into via using either their email address or username. The login details are provided within the view page.
    ...
    function login() {
		...
		if ($this->flexi_auth->ip_login_attempts_exceeded())
		{
			 ...
			 * 
			 * Note: To use this, you will also need to enable the recaptcha
                         *  in 'models/colab_auth_model.php', and 'views/colab/login_view.php'.
			*/
			 ...
			 * Note: To use this example, you will also need to enable 
                         * the math_captcha examples in 'models/colab_auth_model.php', 
		...
		$this->data['message'] = (! isset($this->data['message'])) ? $this->session->flashdata('message') : $this->data['message'];		
                $this->data['titulo'] = "Acceso - Registro";
                
		$this->load->view('colab/login_view', $this->data);
    }
    ...
    function login_via_ajax()
    {
		if ($this->input->is_ajax_request())
		{
			...
			$this->colab_auth_model->login_via_ajax();

			die($this->flexi_auth->is_logged_in());
		}
		else
		{
                        $this->data['titulo'] = "Acceso / Registro con AJAX";
			$this->load->view('colab/login_via_ajax_view', $this->data);
		}
    }
	...
	{       
		...
		{			
                        $form_submitted = TRUE;
			$this->load->model('colab_auth_model');
			$succesful_register = $this->colab_auth_model->register_account();
		}
		
		// Get any status message that may have been set.
		$this->data['message'] = (! isset($this->data['message'])) ? $this->session->flashdata('message') : $this->data['message'];		
                $this->data['titulo'] = 'Registro de usuario';
                
                //if form has been submitted and the registration process was well then show successful registration message 
                if ($form_submitted == TRUE && $succesful_register!=FALSE){
                    $this->load->view('colab/public/successful_register', $this->data);
                }
                //else go to registration form
                else{
                    $this->load->view('colab/public/register_view', $this->data);
	 ...
	 * The default setup of this application "COLABORADORES" requires that new account registrations must be authenticated via email before the account is activated.
	 * In this application , this page is accessed via an activation link in the 'views/includes/email/activate_account.tpl.php' email template.
	 */ 
	function activate_account($user_id, $token = FALSE)
	{
		...
		if ($this->flexi_auth->activate_user($user_id, $token, TRUE))
		{
			$sql = "SELECT upro_first_name, upro_last_name
						FROM collaborators_user_profiles
						WHERE upro_id = " . $user_id;
						...
						$query = $this->db->query($sql);
                            ...
                            if ($query->num_rows()>0)
                            {
                                ...
                                {
                                    $user_data = array(
                                        'name' => $row->upro_first_name,
                                        'surname' => $row->upro_last_name,
										'user_id' => $user_id
                                    );
								}
							}
						//$this->CI->flexi_auth_model->send_email('info@bonisoft.com', 'Registro de NUEVO colaborador', $user_data, $this->CI->auth->email_settings['email_template_directory'] . 'new_colaborator_email.tpl.php');
						$this->flexi_auth->send_email('info@bonisoft.com', 'Registro de NUEVO colaborador','new_colaborator_email.tpl.php',$user_data );
		}
	 ...
	 * In this application , this page is accessed via a link on the login page.
	 */ 
	function resend_activation_token()
	{
		...
		{
			$this->load->model('colab_auth_model');
			$this->colab_auth_model->resend_activation_token();
		}
		
		// Get any status message that may have been set.
		$this->data['message'] = (! isset($this->data['message'])) ? $this->session->flashdata('message') : $this->data['message'];		
                $this->data['titulo'] = 'Reenviar mensaje de activaci&oacute;n';
                
		$this->load->view('colab/public/resend_activation_token_view', $this->data);		
	}
	...
	function forgotten_password()
	{
		...
		{
			$this->load->model('colab_auth_model');
			$this->colab_auth_model->forgotten_password();
		}
		
		// Get any status message that may have been set.
		$this->data['message'] = (! isset($this->data['message'])) ? $this->session->flashdata('message') : $this->data['message'];		
                $this->data['titulo'] = 'Contrase&ntilde;a Olvidada';
                
		$this->load->view('colab/public/forgot_password_view', $this->data);		
	}
	 ...
	 * See the auto_reset_forgotten_password() function below for an example of directly emailing the user a new randomised password.
	 * In this application , this page is accessed via a link in the 'views/includes/email/forgot_password.tpl.php' email template, which must be set to 'auth/manual_reset_forgotten_password/...'.
	 */
	function manual_reset_forgotten_password($user_id = FALSE, $token = FALSE)
	{
		...
		{
			$this->load->model('colab_auth_model');
			$this->colab_auth_model->manual_reset_forgotten_password($user_id, $token);
		}
		...
		$this->load->view('colab/public/forgot_password_update_view', $this->data);
	}
	 ...
	 * In this application , this page is accessed via a link in the 'views/includes/email/forgot_password.tpl.php' email template, which must be set to 'auth/auto_reset_forgotten_password/...'.
	 */
	 ...
	 * In this application , this page is accessed via a link on the colab_header once a user is logged in.
	 */
	function logout() 
	{
	 ...
	 * In this application , this controller method is actually not linked to. It is included here as an example of logging a user out of only their current session.
	 */
	...
	/* 
	 * This function displays a view that shows the privacy agreement (Aviso Proteccion de Datos)
	 * 
	 */
	function lopd() 
	{
		$this->data['titulo'] = 'Aviso de protecci&oacute;n de datos';
		$this->load->view('colab/public/privacy_agreement_view', $this->data);
        }
}
