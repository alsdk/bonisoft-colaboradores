...
*
* Description: Collaborators managament: A full login authorisation and user management 
* software written in PHP using  CodeIgniter framework
* and based on flexi auth (by Rob Hussey),  which itself was based on Ion Auth (By Ben Edmunds)
*  which itself was based on Redux Auth 2 (Mathew Davies)
* Released: 30/01/2014
...
////Send information email to moderator  <---NUEVO
                        /** 
                         *  Obtain more info from colaborator to show in admin email
                         */
//                        $sql_select = array(
//				$this->CI->auth->primary_identity_col,
//				$this->CI->auth->tbl_col_user_account['email']
//			);
//			$sql_where[$this->CI->auth->primary_identity_col] = $user_id;
//			$user = $this->CI->flexi_auth_model->get_users($sql_select, $sql_where)->row();
//                        print_r($sql_select . " <br/> " . $sql_where . " <br/> " . $user);
                        //$email = $user->$this->CI->auth->database_config['user_acc']['columns']['email'];
                        
			//$user_data = array('user_id'  => $user_id);
                                          // 'email'    => $email
//                        //comentamos esto para ver de corregir error en acticación de colaborador
////			$this->flexi_auth->send_email('info@bonisoft.com', 'Registro de NUEVO colaborador', 'new_colaborator_email.tpl.php', $user_data);
//                        $this->flexi_auth_model->send_email('info@bonisoft.com', 'Registro de NUEVO colaborador', 'new_colaborator_email.tpl.php', $user_data);
//			
	...
	public function resend_activation_token($identity) 
	{
		...
		else if ($this->CI->flexi_auth_model->deactivate_user($user_id))
		{
			...
			// Set email data.
			$email_to = $email;
			$email_title = ' - Active su cuenta de COLABORADOR';
	...
	public function insert_user($email, $username = FALSE, $password, $user_data, $group_id = FALSE, $activate = FALSE) 
	{
		...
		if ($user_id)
		{
			...
			{
				// Set email data. //$email; --> OBTENER email admin (rol 2)
				$email_to = $email;
                /** 
                 * @author: Jose Miguel Bonilla
                 * @todo Pending OBTENER el email del MODERADOR (rol 2) desde la base de datos
                 */
				$email_title = '- Valide su email'; //' - Account Activation';
				...
				if ($this->CI->flexi_auth_model->send_email($email_to, $email_title, $user_data, $template))
				{
					$this->CI->flexi_auth_model->set_status_message('activation_email_successful', 'config');
                                        ...
                                        /**
                                         *  Send information email to  moderator by JOSE MIGUEL BONILLA
                                         * @todo: ADMIN rol
                                         * @author Jose Miguel Bonilla <info@bonisoft.com>
                                         *  Error $this->CI->flexi_auth_model->send_email
                                         * cambiado a $this->CI->flexi_auth_model->send_email
                                         */

                                        //$this->CI->flexi_auth_model->send_email('info@bonisoft.com', 'Registro de NUEVO colaborador', 'new_colaborator_email.tpl.php', $user_data);
